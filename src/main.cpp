...
// Draw the middle road marking (dashed white line)
for (int i = 0; i < 800; i += 100) {
    glBegin(GL_QUADS);
        glColor3f(1.0, 1.0, 1.0);
        glVertex2f(i, 225);   // Bottom-left (adjusted to new road position)
        glVertex2f(i + 50, 225);
        glVertex2f(i + 50, 230);
        glVertex2f(i, 230);
    glEnd();
...
// Adding perspective-aligned planks
...
for (int i = 0; i <= 145; i += 15) { // Adjust stripe spacing
    ...
    glBegin(GL_LINES);
    glVertex2f(leftX, i);   // Start of plank line (left side)
    glVertex2f(rightX, i);  // End of plank line (right side)
    glEnd();
}
...
glEnd();
glBegin(GL_QUADS);
    glColor3ub(25, 25, 25); // Dark grey/black pole
    glVertex2f(349, 150);  // Left side
    glVertex2f(351, 150);  // Right side (thinner pole)
    glVertex2f(351, 300);
    glVertex2f(349, 300);
glEnd();

// Draw Traffic Light Box (Adjusted Position)
glBegin(GL_QUADS);
    glColor3ub(0, 0, 0); // Black box

    glVertex2f(342, 300); // Move left boundary closer to center
    glVertex2f(358, 300); // Move right boundary closer to center
    glVertex2f(358, 350); // Keep the height the same
    glVertex2f(342, 350); // Keep the height the same
glEnd();
...
if (activeLight == 'R') {
        ...
        for (int i = 0; i <= 360; i++) {
            ...
            glVertex2f(350 + cos(angle) * 7, 340 + sin(angle) * 7);
        }
        glEnd();
    }
    ...
    if (activeLight == 'Y') {
        ...
        for (int i = 0; i <= 360; i++) {
            float angle = i * 3.14159f / 180;
            glVertex2f(350 + cos(angle) * 7, 325 + sin(angle) * 7);
        }
        glEnd();
    }
    if (activeLight == 'G') {
        ...
        for (int i = 0; i <= 360; i++) {
            float angle = i * 3.14159f / 180;
            glVertex2f(350 + cos(angle) * 7, 310 + sin(angle) * 7);
        }
        glEnd();
    }
    ...
    // Draw Pole (Adjusted to End at the Box)
glBegin(GL_QUADS);
    glColor3ub(25, 25, 25); // Dark grey/black pole
    glVertex2f(334, 300);  // Left side
    glVertex2f(336, 300);  // Right side
    glVertex2f(336, 400);  // Stop at the bottom of the box
    glVertex2f(334, 400);
glEnd();

// Adjusted Traffic Light Box (Starts Where the Pole Ends)
glBegin(GL_QUADS);
    glColor3ub(0, 0, 0); // Black box
    glVertex2f(325, 400);  // Left boundary
    glVertex2f(345, 400);  // Right boundary
    glVertex2f(345, 450);  // Adjusted height
    glVertex2f(325, 450);
glEnd();


    // Show the active light
    if (activeLight == 'R') glColor3ub(255, 0, 0);
    else glColor3ub(50, 50, 50); // Dimmed
    glBegin(GL_TRIANGLE_FAN);
    glVertex2f(335, 440);
    for (int i = 0; i <= 360; i++) {
        float angle = i * 3.14159f / 180;
        glVertex2f(335 + cos(angle) * 7, 440 + sin(angle) * 7);
    }
    glEnd();

    if (activeLight == 'Y') glColor3ub(255, 255, 0);
    else glColor3ub(50, 50, 50); // Dimmed
    glBegin(GL_TRIANGLE_FAN);
    glVertex2f(335, 425);
    for (int i = 0; i <= 360; i++) {
        float angle = i * 3.14159f / 180;
        glVertex2f(335 + cos(angle) * 7, 425 + sin(angle) * 7);
    }
    glEnd();

    if (activeLight == 'G') glColor3ub(0, 255, 0);
    else glColor3ub(50, 50, 50); // Dimmed
    glBegin(GL_TRIANGLE_FAN);
    glVertex2f(335, 410);
    for (int i = 0; i <= 360; i++) {
        float angle = i * 3.14159f / 180;
        glVertex2f(335 + cos(angle) * 7, 410 + sin(angle) * 7);
    }
    glEnd();
...
//lamp 4
 glBegin(GL_QUADS);
    ...
    glBegin(GL_TRIANGLE_FAN);
        ...
        for (int i = 0; i <= 360; i++) {
            float angle = i * 3.14159f / 180;
            glVertex2f(z + cos(angle) * 5, 275 + sin(angle) * 5);
        }
    glEnd();
